...
export async function GET() {
  ...
  try {
    ...
    const items = (json.data || []).map((raw) => {
      ...
      const coverUrl = isRecord(cover) && typeof cover.url === 'string' ? toAbsoluteStrapiUrl(cover.url) : null;
      ...
      return {
        ...
        publishedAt:
          typeof er.publishedAt === 'string' ? er.publishedAt : er.publishedAt === null ? null : null,
